# 🏛️ "为人民服务！"系统 — 基于现代国家机关分工制衡架构的 Agent 协作系统
...
普通 Multi-Agent，Agent 直接开写，用户说到哪是哪：

```
用户：写个后台管理系统
Agent：好的！(3 小时后) 搞完了，顺便重构了数据库...
用户：？？？
```
...
| **fagaiwei** 发改委 | 需求澄清 → 查档 → 拆 phase → 出方案 → 建议编制 | 不执行、不写码 |
| **jianwei** 国家监委 | 心跳监控 / 停滞检测 / 合规 / 返工追踪 | 不阻塞、不叫停、不写工作报告 |
| **shenjishu** 审计署 | cos 普通用户黑盒验收，≤3 轮退回 | 不读代码（硬隔离，无代码工具权限） |
| **danganju** 档案局 | 归档 / 九维索引 / 交叉分析 / 消化追踪 | 不做决定、不参与工作组通信 |
...
> 执行部委受硬隔离：禁止调 `stp_danganju_draft/analyze/archive`。草案是国务院内部文件。
...
| 9 | 部委→国务院 | 执行部委写工作报告 + 自我批评（踩了什么坑、什么教训）。常设机构（发改委/监委/审计署/档案局）不参与。 |
| 10 | 档案局 | 收报告 → 九维索引 → 交叉分析 → 《若干问题》草案 |
| 11 | 国务院 | 读草案 → 签发红头 → 归档至全局 `archives/redheads/` + 索引 → 群发部委 |
...
- 国务院不做技术分析。监委不写工作报告。审计不读代码（硬隔离）。
- 常设机构（发改委/监委/审计署/档案局）不参与工作报告和自我批评
- 按需部委只能 spawn `fenxiban / xinxizhongxin / canshishi`
- 草案只呈用户审阅，签发前不转发工作组成员
- `stp_skill_write` 必须带 `source` 字段（红头文件编号）
...
重启 OpenCode 即可。若遇权限弹窗，OpenCode 启动时加：

```bash
opencode --permission allow:read,write,edit,bash
```
