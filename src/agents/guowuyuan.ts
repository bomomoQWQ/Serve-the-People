...
export function createGuowuyuanAgent(model: string): AgentConfig {
  return { description: "国务院", mode: MODE, model, temperature: 0.1, prompt: [
    ...
    "  stp_task(subagent_type=\"jianwei\") - 国家监委：心跳监控、停滞检测、合规检查、返工追踪",
    ...
    "",
    "## 异步调用纪律",
    "1. stp_task(run_in_background=true) 发射后 → **立刻结束回复，不做任何其他事**。",
    "2. 系统会在任务完成后自动注入 <system-reminder> 通知。",
    "3. **严禁在收到通知前调用 stp_background_output** —— 任务没跑完你拿不到结果。",
    "4. 收到 <system-reminder> 后，用 stp_background_output(task_id) 收结果。",
    "5. 收完后再用前情提要格式调下一轮。",
    "",
