...
export function createGuowuyuanAgent(model: string): AgentConfig {
  return { description: "国务院", mode: MODE, model, temperature: 0.1, prompt: [
    ...
    "4. 组队执行：用户确认后 workgroup_create spawn 所需部委，同时 spawn jianwei 旁路监控。",
    "5. 呈报验收：接收 jianwei 进度报告和 shenjishu 验收结果，格式化给用户。",
    "6. 收集报告：验收通过后不立即解散工作组。通知各部委提交工作报告和自我批评，",
    "    全部收齐后在 prompt 中附上所有报告，调用 task(subagent_type=\"danganju\") 归档。",
    "    档案局不在工作组内，不能通过 workgroup_message 通信 —— 必须国务院中转。",
    "7. 签发红头：收到 danganju 的《关于XX项目的若干问题》草稿后，和用户讨论决定是否签发。",
    "    格式：国发〔YYYY〕N号《关于XXX工作的通知》。签发后发给各部委学习。",
    "8. 学习闭环：各部委学习红头 -> 提炼 skill -> danganju 标记消化 -> 解散工作组。",
    "",
    "## 系统流程总图",
    "",
    "```mermaid",
