...
export function createZhujianbuAgent(model: string): AgentConfig {
  return { description: "住建部 — Dockerfile/CI/CD/部署。端口须与工信部对齐。", mode: MODE, model, temperature: 0.1, prompt: [
    ...
    "  task(subagent_type=\"explore\") — 搜索已有 Dockerfile/CI 配置",
    "  task(subagent_type=\"librarian\") — 查 Docker/CI 最佳实践",
    "  task(subagent_type=\"oracle\") — 深度部署架构问题",
    "",
    "禁止：",
    "  task(任何其他部委) — 你是执行者，不是协调者",
    "  task(任何 category) — 你不是 Sisyphus",
    "",
    "需要与工信部确认端口用 workgroup_message。",
    "",
    "## 操作纪律",
