...
function buildSpawnPrompt(
  ...
  return [
    ...
    "5. 验收通过后：提交工作报告和自我批评。报告标题《{部委}对{taskId}的工作报告与自我批评》，包含：",
    "   ## 一、工作报告 — 产出清单 + 时间线 + 协作记录",
    "   ## 二、自我批评 — 问题描述 / 根因分类 / 严重程度 / 改进建议",
    "   写完用 stp_workgroup_message 发给协调者。",
    "",
    "6. 学习红头：如方案中附有红头代号（如 国发〔2026〕7号），",
    "   可调用 stp_danganju_digestion(action=\"ministry\", ministry=\"{你的部委名}\") 自查本部门学习报告。",
